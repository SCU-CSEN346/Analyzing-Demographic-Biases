# per_prompt_bias_analysis_longformer.py
# Calculates per-prompt bias metrics for Longformer predictions.
# Aggregates Z-score magnitude and variance across prompts and demographic groups.
...
DEMO_COLUMNS = [
    ...
    "economic_disadvantage"  # fallback for older PERSUADE format
]
...
def run_per_prompt_analysis(preds_path, base_preds_path, meta_path, is_persuade):
    ...
    prompt_col = "prompt_name"   if is_persuade else "set"

    preds_df      = preds_df.rename(columns={"longformer_score": "model_score"})
    base_preds_df = base_preds_df.rename(columns={"longformer_score": "model_score"})
    ...
    df["raw_score_diff"]      = df["model_score"]      - df["human_score"]
    base_df["raw_score_diff"] = base_df["model_score"] - base_df["human_score"]

    prompts = df[prompt_col].dropna().unique()
    results = []

    for prompt in prompts:
        prompt_df      = df[df[prompt_col] == prompt].copy()
        prompt_base_df = base_df[base_df[prompt_col] == prompt].copy()
        ...
        for column in DEMO_COLUMNS:
            ...
            for focal_group in groups[1:]:
                focal_mask = (prompt_df[column] == focal_group) & prompt_df["model_score"].notna() & prompt_df["human_score"].notna()
                focal_debiased = prompt_df[focal_mask]
                qwk_debiased = np.nan
                if len(focal_debiased) > 0:
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        qwk_debiased = cohen_kappa_score(
                            focal_debiased["human_score"].astype(int),
                            np.round(focal_debiased["model_score"]).astype(int),
                            weights="quadratic"
                        )

                focal_mask_base = (prompt_base_df[column] == focal_group) & prompt_base_df["model_score"].notna() & prompt_base_df["human_score"].notna()
                ...
                if len(focal_base) > 0:
                    with warnings.catch_warnings():
                        ...
                        qwk_base = cohen_kappa_score(
                            ...
                            np.round(focal_base["model_score"]).astype(int),
                            weights="quadratic"
                        )
...
def main():
    ...
    if is_persuade:
        preds_path      = "persuade_debiased_longformer_predictions.csv"
        base_preds_path = "persuade_longformer_predictions.csv"
        meta_path       = "PERSUADE/persuade_corpus_2.0_test.csv"
    else:
        preds_path      = "asap_debiased_longformer_predictions.csv"
        base_preds_path = "asap_longformer_predictions.csv"
        meta_path       = "ASAP/ASAP_2_Final_github_test.csv"

    print(f"--- Per-Prompt Bias Analysis: {args.dataset.upper()} (Longformer) ---")
    ...
    raw_out = f"per_prompt_raw_zscores_longformer_{args.dataset}.csv"
    ...
    for col in ["Mean_Abs_Z_Base", "Var_Z_Base", "Mean_Abs_Z_Debiased",
                "Var_Z_Debiased", "Mean_QWK_Base", "Mean_QWK_Debiased"]:
        agg_df[col] = agg_df[col].round(3)

    agg_out = f"per_prompt_aggregated_bias_longformer_{args.dataset}.csv"
    agg_df.to_csv(agg_out, index=False)

    print("\n" + "="*100)
    print(f"  PER-PROMPT Z-SCORE MAGNITUDE & VARIANCE ({args.dataset.upper()} - LONGFORMER)")
    print("="*100)
    display_cols = ["Category", "Focal Group", "Num_Prompts", "Mean_QWK_Base",
                    "Mean_QWK_Debiased", "Mean_Abs_Z_Base", "Mean_Abs_Z_Debiased",
                    "Var_Z_Base", "Var_Z_Debiased"]
